...
class TestFriends(unittest.TestCase):
    ...
    def _init_friend_worker(self) -> FriendWorker:
        return FriendWorker(USER_2)

    def test_init_friend_worker_type_error(self):
        user = 4
        self.assertRaises(TypeError, FriendWorker, user)
    ...
    def test_get_friend_db_object(self):
        ...
        # friendship
        friend_worker_1 = FriendWorker(USER_2)
        friend_worker_2 = FriendWorker(USER_3)

        friendship_1 = friend_worker_1._get_friend_db_object(USER_3)
        friendship_2 = friend_worker_2._get_friend_db_object(USER_2)
        self.assertEqual(friendship_1.id, friendship_2.id, 1)

    @DB_SESSION_RETRYABLE
    def test_get_friend_db_object_no_friendship(self):
        friend_worker = FriendWorker(USER_1)
        friendship = friend_worker._get_friend_db_object(USER_3)
        self.assertIsNone(friendship)

    @DB_SESSION_RETRYABLE
    def test_create_friend_entry(self):
        friend_worker = FriendWorker(USER_1)
        friend_worker.create_friend_entry(USER_3)
        friendship = friend_worker._get_friend_db_object(USER_3)
    ...
    def test_accept_friend_request_error_1(self):
        ...
        '''
        friend_worker = FriendWorker(USER_3)
        self.assertRaises(
            BadRequestError, friend_worker.accept_friend_request, USER_4
        )
    ...
    def test_accept_friend_request_error_3(self):
        ...
        '''
        friend_worker = FriendWorker(USER_2)
        self.assertRaises(
            BadRequestError, friend_worker.accept_friend_request, USER_3
        )
    ...
    def test_accept_friend_request(self):
        ...
        This error is because the there is no friend request
        '''
        friend_worker = FriendWorker(USER_4)
        friend_worker.accept_friend_request(USER_3)
        friendship = friend_worker._get_friend_db_object(USER_3)
    ...
    def test_delete_friend(self):
        ...
        '''
        friend_worker = FriendWorker(USER_2)
        friend_worker.delete_friend(USER_3)
        self.assertIsNone(friend_worker._get_friend_db_object(USER_3))
    ...
    def test_delete_friend_who_arent_friends(self):
        ...
        '''
        friend_worker = FriendWorker(USER_1)
        self.assertRaises(BadRequestError, friend_worker.delete_friend, USER_3)
    ...
    def test_search_for_users(self):
        expected_users = PaginatedUserSearch(
            user_data=[
                {
                    'id': 4,
        ...
        search_term = SearchUsers(term='random mate', page_count=1)
    ...
    def test_search_for_users_unknown_name(self):
        ...
        users = search_for_users(USER_2, search_term)
        self.assertEqual(
            users, PaginatedUserSearch(user_data=[], total_pages=0)
        )
